...
COURSE_STYLE_TEMPLATES: dict[CourseContentStyle, CourseStyleTemplate] = {
    ...
    "story": CourseStyleTemplate(
        ...
        preparation_rules=(
            ...
            "背景引导采用一个轻量且与主题直接相关的事件，不展开完整知识；关键词按02使用顺序排列，每个词分别给出准确解释、场景对应和本节作用。",
            "不生成3至5个问题清单、学习目标播报、例题、关系链、口诀或重复总结；最多留下1个由02立即接住的自然问题。",
        ),
        explanation_rules=(
            "02-讲解的职责是讲清结论为什么成立、知识怎样连起来；开头必须接住01的同一事件和最后一句，不使用文档导航式套话。",
            "正文须包含4至7个问题式或事件式二级标题，每个标题形成一个 explanationBeats 项并沿因果主线推进；每一项都写清因果解释、准确结论及必要边界。",
            "结尾用自然事件进展引出03-例题。",
        ),
        example_rules=(
            "03-例题的职责是把02中的判断方法放进题目；开头承接02的事件进展，结构固定为1个主故事综合例题加2至3道独立变式。",
