...
- [ ] 改善失败/部分成功/继续生成/取消/恢复的前端状态展示
  - 2026-08-28 生成体验审计：P0 接入 active-job（刷新后恢复任务）；轮询通信错误不得伪装成真实生成失败
  - P1 直接展示 job.modelUsage 的 stage/taskId/updatedAt、真实任务尝试预算、最近进展与运行健康度；区分“请求停止”与“已停止”
  - P2 保留最近运行摘要、错误分类与定向操作；改为单飞轮询；积累历史后再做 ETA 区间，不展示未经校准的成功率
- [ ] 课程生成效率审计后的瘦身：一节课当前串行生成讲义+自测（各最多2次模型调用），故事版硬结构/考点-自测全覆盖易触发整包修复；优先研究局部确定性修复或一次合并生成，保持事实/答案硬校验与显式失败原则
- [ ] 补本地首次安装、依赖检查、启动说明
